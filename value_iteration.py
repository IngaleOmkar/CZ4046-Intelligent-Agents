...
#     grid_env: Grid = None
#     __utility_list: List[List[List[Utility]]] = []
#     __grid: List[List[State]] = None
#     __iterations: int = 0
#     __convergence_threshold: float = None
#     __value_iteration: bool = True

#     @staticmethod
#     def initial_setup(grid_env: Grid):
#         ValueIteration.grid_env = grid_env
#         ValueIteration.__grid = grid_env.get_grid()

#         ValueIteration.run_value_iteration(ValueIteration.__grid)

#         ValueIteration.display_results()

#         WriteOutput.write_output(ValueIteration.__utility_list, "value_iteration_utilities")

        

#     @staticmethod
#     def display_results():
#         optimal_policy = ValueIteration.__utility_list[-1]

#         ShowGrid.display_grid(ValueIteration.__grid)

#         ShowGrid.display_setup(ValueIteration.__value_iteration, ValueIteration.__convergence_threshold)

#         ShowGrid.display_iteration_count(ValueIteration.__iterations)

#         ShowGrid.display_utilities(ValueIteration.__grid, optimal_policy)
...
#     @staticmethod
#     def run_value_iteration(grid: List[List[State]]):
#         current_utility_arr = [[Utility() for _ in range(grid_constants.TOTAL_COLS)] for _ in range(grid_constants.TOTAL_ROWS)]
#         new_utility_arr = [[Utility() for _ in range(grid_constants.TOTAL_COLS)] for _ in range(grid_constants.TOTAL_ROWS)]
...
#         __convergence_threshold = iteration_constants.MAX_DISCOUNTED_ERROR * ((1.000 - iteration_constants.DISCOUNT_FACTOR) / iteration_constants.DISCOUNT_FACTOR)

#         first_pass = True

#         while(first_pass or (delta >= __convergence_threshold)):
#             first_pass = False

#             UtilityController.update_utility(new_utility_arr, current_utility_arr)

#             delta = sys.float_info.min

#             curr_util_arr_cpy = []

#             UtilityController.update_utility(curr_util_arr_cpy, current_utility_arr)

#             __utility_list.append(curr_util_arr_cpy)
...
#             ValueIteration.__iterations += 1

# if __name__ == "__main__":
#     ValueIteration.initial_setup(Grid())
